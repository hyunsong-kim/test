# python_consumer.py
# RabbitMQ -> Oracle (batch insert) consumer
# Async RabbitMQ (aio-pika) + oracledb executemany batching
...
RMQ_URL = os.getenv("RMQ_URL", "amqp://guest:guest@localhost:5672/%2f")
RMQ_QUEUE = os.getenv("RMQ_QUEUE", "bulk.q")
FLUSH_SIZE = int(os.getenv("FLUSH_SIZE", "5000"))
FLUSH_INTERVAL_MS = int(os.getenv("FLUSH_INTERVAL_MS", "1000"))
PREFETCH = int(os.getenv("PREFETCH", "200"))

ORA_DSN = os.getenv("ORA_DSN", "localhost:1521/ORCLPDB1")
ORA_USER = os.getenv("ORA_USER", "app")
ORA_PASS = os.getenv("ORA_PASS", "app")
    ...
    async def on_message(self, msg: aio_pika.IncomingMessage):
        async with msg.process(ignore_processed=True, requeue=False):
            try:
                ...
                if key and key in self.seen_keys:  # in-proc idempotency
                    return
                if key:
